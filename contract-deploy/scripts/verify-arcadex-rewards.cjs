...
const DEPLOYMENT_FILE = path.resolve(
  __dirname,
  "../../deployments/arcadex-rewards-celo-mainnet.json"
);
const CONTRACT_IDENTIFIER = "contracts/ArcadeXRewards.sol:ArcadeXRewards";
...
function findBuildInfo() {
  ...
  for (const file of files) {
    ...
    if (buildInfo.input?.sources?.["contracts/ArcadeXRewards.sol"]) {
      return buildInfo;
    }
  }

  throw new Error(
    "No build-info artifact found for contracts/ArcadeXRewards.sol"
  );
}

function encodeConstructorArgs(deployment) {
  const signer =
    (deployment.constructorArgs && deployment.constructorArgs[0]) ||
    deployment.eligibilitySigner;
  if (!signer) {
    throw new Error("Missing constructor eligibility signer in deployment file");
  }
  const iface = new Interface([
    "constructor(address initialEligibilitySigner)",
  ]);
  return iface.encodeDeploy([signer]).slice(2);
}
...
async function verifyOnSourcify(address, txHash) {
  ...
  if (submit.status !== 200 && submit.status !== 202) {
    if (
      submit.status === 409 &&
      submit.body?.customCode === "already_verified"
    ) {
...
async function verifyOnCeloscan(address, deployment) {
  if (!apiKey) {
    return false;
  }
  ...
  const constructorArguements = encodeConstructorArgs(deployment);
...
async function main() {
  ...
  if (apiKey) {
    celoscanOk = await verifyOnCeloscan(address, deployment);
  } else {
    console.log(
